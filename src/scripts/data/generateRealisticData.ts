...
import { CargoModel } from '../../infrastructure/database/models/CargoModel';

// Perfis de vendedores
interface PerfilVendedor {
    nome: string;
    frequenciaVendas: number; // 0 a 1 (probabilidade de vender em um dia)
    fatorDesempenho: number; // 0.5 a 1.5 (quanto mais alto, mais vende)
    consistencia: number; // 0.5 a 1.5 (quanto mais alto, mais consistente nas vendas)
    evolucao: number; // 0.8 a 1.2 (quanto mais alto, mais evolui ao longo do tempo)
}

const perfisVendedores: PerfilVendedor[] = [
...
async function generateData() {
    try {
        ...
        await TemaModel.deleteMany({});
        await CargoModel.deleteMany({});

        // Cria temas
        console.log('🎨 Criando temas...');
        const temas = [
            new Tema(uuidv4(), "Gladiador", "Equipe Gladiador - Força e Determinação", "#FF0000"),
            new Tema(uuidv4(), "Arqueiro", "Equipe Arqueiro - Precisão e Foco", "#00FF00"),
            new Tema(uuidv4(), "Titã", "Equipe Titã - Poder e Resistência", "#0000FF"),
            new Tema(uuidv4(), "Lutador", "Equipe Lutador - Coragem e Persistência", "#FFA500")
        ];
        await TemaModel.insertMany(temas);
        console.log('✅ 4 temas criados');

        // Cria cargos
        console.log('👔 Criando cargos...');
        const cargos = [
            new Cargo(uuidv4(), "Gerente", "Gerente de Equipe", "gerente"),
            new Cargo(uuidv4(), "Capitão", "Capitão de Equipe", "capitao"),
            new Cargo(uuidv4(), "Vendedor", "Vendedor de Equipe", "vendedor")
        ];
        await CargoModel.insertMany(cargos);
        ...
        const equipes = nomesEquipes.map((nome, index) => new Equipe(
            uuidv4(),
            nome,
            `PDV ${nome}`,
            'São Paulo',
            ...
            `Capitão ${nome}`,
            `(11) 9${Math.floor(Math.random() * 9000) + 1000}-${Math.floor(Math.random() * 9000) + 1000}`,
            temas[index].id // Usa o ID do tema correspondente
        ));

        // Bulk insert para equipes
        await EquipeModel.insertMany(equipes);
        ...
        for (let i = 0; i < equipes.length; i++) {
            ...
            for (const perfil of vendedoresEquipe) {
                vendedores.push(new Vendedor(
                    uuidv4(),
                    perfil.nome,
                    equipe.id,
                    `${perfil.nome.toLowerCase().replace(' ', '.')}@nimage.com`,
                    `(11) 9${Math.floor(Math.random() * 9000) + 1000}-${Math.floor(Math.random() * 9000) + 1000}`,
                    100000,
                    'Vendedor Senior'
                ));
        ...
        await VendedorModel.insertMany(vendedores);
        ...
        await MetaModel.insertMany(metas);
        ...
        for (const vendedor of vendedores) {
            ...
            for (const data of datas) {
                ...
                if (Math.random() > perfil.frequenciaVendas) continue;
        ...
        for (let i = 0; i < atividades.length; i += batchSize) {
            ...
            await AtividadeModel.insertMany(batch);
